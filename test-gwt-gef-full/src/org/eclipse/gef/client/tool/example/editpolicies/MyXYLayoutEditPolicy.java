package org.eclipse.gef.client.tool.example.editpolicies;

import org.eclipse.draw2d.geometry.Dimension;
import org.eclipse.draw2d.geometry.Rectangle;
import org.eclipse.gef.EditPart;
import org.eclipse.gef.Request;
import org.eclipse.gef.client.tool.example.model.command.CreateCommand;
import org.eclipse.gef.client.tool.example.model.command.MoveCommand;
...
public class MyXYLayoutEditPolicy extends XYLayoutEditPolicy {
	...
	protected Command createChangeConstraintCommand(EditPart child,
		...
		Command command = new MoveCommand(child.getModel(),
	...
	protected Command getCreateCommand(CreateRequest request) {
		CreateCommand command = new CreateCommand(getHost().getModel(),
				request.getNewObject());
		Dimension size = request.getSize();
		if (size == null) {
			command.setLocation(request.getLocation());
		} else {
			command.setLocation(new Rectangle(request.getLocation(), request
					.getSize()));
		}
		return command;
	}
